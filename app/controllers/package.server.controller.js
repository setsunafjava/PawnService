...
exports.create = function(req,res){
    ...
    dbconn.query('INSERT INTO packages SET ?', req.body, function(err,res){
        if(err){
...
exports.list = function(req,res){
    dbconn.query('SELECT * FROM packages where project_id = ? ',[req.params.packageforprojectId],function(err, records){
        if(err){
            console.log(err);
            return res.status(400).send({
                message : getErrorMessage(err)
...
exports.update = function (req,res) {
    ...
    dbconn.query('"UPDATE packages SET ? WHERE id= ?', [req.body, req.records.id], function(err, result){
        if(err){
            return res.status(400).send({
                message : getErrorMessage(err)
            });
...
exports.delete = function(req,res){
    dbconn.query('DELETE FROM packages WHERE id = ?', [req.records.id], function(err, result){
        if(err){
            return res.status(400).send({
                message: getErrorMessage(err)
            });
